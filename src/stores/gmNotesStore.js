  ...
  mutations: {
    updateNotes(state, changesObj) {
      if (Object.hasOwnProperty.call(changesObj, "notes")) {
        if (
          JSON.stringify(Object.keys(changesObj["notes"])).length >
          JSON.stringify(changesObj["keysOfNotes"]).length
        ) {
          state["keysOfNotes"] = [
            ...state["keysOfNotes"],
            Object.keys(changesObj["notes"]).slice(-1).pop(),
          ];
          ...
          changesObj["keysOfNotes"] = changesObj["keysOfNotes"].filter((item) =>
            originArray.includes(item)
          );
