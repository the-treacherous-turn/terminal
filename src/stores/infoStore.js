  ...
  mutations: {
    updateInfo(state, changesObj) {
      if (Object.hasOwnProperty.call(changesObj, "customCampaignNotes")) {
        if (
          JSON.stringify(Object.keys(changesObj["customCampaignNotes"]))
            .length > JSON.stringify(changesObj["keysOfCampaignNotes"]).length
        ) {
          state["keysOfCampaignNotes"] = [
            ...state["keysOfCampaignNotes"],
            Object.keys(changesObj["customCampaignNotes"]).slice(-1).pop(),
          ];
          for (const key in changesObj) {
            if (
              Object.hasOwnProperty.call(changesObj, key) &&
              key !== "keysOfCampaignNotes" &&
              key !== "keysOfInPlayNotes"
            ) {
          ...
          changesObj["keysOfCampaignNotes"] = changesObj[
            "keysOfCampaignNotes"
          ].filter((item) => originArray.includes(item));
      ...
      if (Object.hasOwnProperty.call(changesObj, "customInPlayNotes")) {
        if (
          JSON.stringify(Object.keys(changesObj["customInPlayNotes"])).length >
          JSON.stringify(changesObj["keysOfInPlayNotes"]).length
        ) {
          state["keysOfInPlayNotes"] = [
            ...state["keysOfInPlayNotes"],
            Object.keys(changesObj["customInPlayNotes"]).slice(-1).pop(),
          ];
          for (const key in changesObj) {
            if (
              Object.hasOwnProperty.call(changesObj, key) &&
          ...
          changesObj["keysOfInPlayNotes"] = changesObj[
            "keysOfInPlayNotes"
          ].filter((item) => originArray.includes(item));
  ...
  actions: {
    async listenToFBInfo({ commit }) {
      ...
      onValue(refs.info, (snapshot) => {
        ...
        if (data.customCampaignNotes === undefined)
          data.customCampaignNotes = {};
        if (data.customInPlayNotes === undefined) data.customInPlayNotes = {};
        if (data.keysOfCampaignNotes === undefined)
          data.keysOfCampaignNotes = [];
    ...
    async updateCustomCampaignNote({ commit, state }, { noteID, changesObj }) {
      ...
      await update(refs.customCampaignNotes, {
        [noteID]: state.customCampaignNotes[noteID],
      });
    },
    ...
    async updateCustomInPlayNote({ commit, state }, { noteID, changesObj }) {
      ...
      await update(refs.customInPlayNotes, {
        [noteID]: state.customInPlayNotes[noteID],
      });
    },
