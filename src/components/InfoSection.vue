  ...
  <div class="relative w-full h-full md:grid md:grid-cols-2 md:gap-4">
    <div class="relative w-full h-full pb-10 overflow-y-scroll">
      <div class="relative w-full max-w-2xl p-4 form-control">
        ...
        <draggable
          :list="keysOfCampaignNotes"
          class="w-full"
          @change="
            keysOfCampaignNotes !== []
              ? campaignMove('keysOfCampaignNotes', keysOfCampaignNotes)
              : ''
          "
          item-key="id"
          handle=".handle"
          ...
          <template #item="{ element }" :key="key">
            <div>
              <label class="label">
                ...
                <font-awesome-icon
                  :icon="['fas', 'trash-can']"
                  class="text-base cursor-pointer text-primary-content hover:text-white"
    ...
    <div class="relative w-full h-full pb-10 overflow-y-scroll">
      <div class="relative w-full max-w-2xl p-4 form-control">
        ...
        ></textarea>
        <draggable
          :list="keysOfInPlayNotes"
          class="w-full"
          @change="
            keysOfInPlayNotes !== []
              ? campaignMove('keysOfInPlayNotes', keysOfInPlayNotes)
              : ''
          "
          item-key="id"
          handle=".handle"
...
export default {
  ...
  methods: {
    ...
    updateCustomCampaignNoteField(key, field, event) {
      ...
      const changesObj = {};
      changesObj[field] = value;
      this.$store.dispatch("updateCustomCampaignNote", {
        noteID: key,
        changesObj,
      });
    },
    ...
    updateCustomInPlayNoteField(key, field, event) {
      ...
      const changesObj = {};
      changesObj[field] = value;
      this.$store.dispatch("updateCustomInPlayNote", {
        noteID: key,
        changesObj,
      });
    },
